...
- `__init__(max_context_age_seconds=7776000, max_messages_per_context=50, max_channel_context_messages=20)`: 初期化
...
- `add_message(message)`: メッセージをコンテキストに追加
  ...
  - スレッドに属さないメッセージはチャンネルコンテキストに追加
  - スレッドメッセージ追加時、スレッドコンテキストが新規の場合は親メッセージをチャンネルコンテキストからコピー
- `cleanup_expired_contexts()`: 期限切れのコンテキストをクリーンアップし、削除数を返す
- `_make_context_key(channel_id, thread_ts)`: コンテキストキーを生成
- `_add_to_context(channel_id, thread_ts, message)`: 指定コンテキストにメッセージを追加（共通処理）
- `_copy_parent_to_thread_context(channel_id, thread_ts)`: チャンネルコンテキストから親メッセージを探してスレッドコンテキストにコピー
- `_is_context_expired(context)`: コンテキストが期限切れか判定
- `get_statistics()`: 統計情報を取得（コンテキスト総数、設定値等）

#### 親メッセージのスレッドコンテキストへのコピー

Slackのイベント仕様上、親メッセージに返信がついても親メッセージのイベントは再発生しません。そのため、返信メッセージが追加される際にスレッドコンテキストが新規作成される場合、チャンネルコンテキストから親メッセージ（`message_id == thread_ts`）を探してスレッドコンテキストにコピーします。
...
返信投稿（スレッドコンテキスト新規）
    ↓
親メッセージをチャンネルコンテキストから探す
    ↓
スレッドコンテキストに親メッセージをコピー
    ↓
返信メッセージをスレッドコンテキストに追加
```
...
- 親メッセージがない場合でもエラーにならないこと
- 2つ目以降の返信で親メッセージが重複しないこと
- 古いコンテキストが削除されること
- メッセージ数の制限が機能すること
- クリーンアップが正しく動作すること
...
- [x] `services/response_judge.py`の実装
- [x] 基本条件チェック
- [x] 反応状況をLLMに渡す
- [x] LLMによる判定呼び出し
- [x] テストコードの作成
...
- [x] `services/response_generator.py`の実装
- [x] LLMによる応答生成
- [x] 応答の後処理
- [x] フォールバック応答
- [x] テストコードの作成
...
- [x] `services/context_manager.py`の実装
- [x] インメモリストレージ
- [x] メッセージ追加・取得
- [x] 期限切れコンテキストのクリーンアップ
- [x] テストコードの作成

### タスク4: 反応待機制御

- [x] `services/delay_controller.py`の実装
- [x] スケジューリング機能
- [x] タスクのキャンセル
- [x] 非同期コールバック実行
- [x] テストコードの作成
