...
#### 2.2.6 スマート応答判定
- **FR-601**: 返答しないと判定したメッセージに対して、一定時間経過後に再判定する
  - 条件: 固定時間経過 + 新しいメッセージがない場合
  - 最大再判定回数を設定可能
- **FR-602**: 返信先（スレッド/チャンネル）を判定する
...
|----------|------|---------|--------|
| 1.0 | 2024-12-21 | 初版作成 | - |
| 1.1 | 2025-12-26 | FR-001をAppがinviteされたチャンネル監視に変更、FR-006aチャンネル履歴参照を追加 | - |
| 1.2 | 2025-12-27 | Phase 2要件変更: FR-601〜FR-603追加、LiteLLM直接使用、DB移行をPhase 3に延期 | - |
